        ...
        char *FileName)
{
    ...
    if (Contents.Size != 0) {
        ...
        // the red green and blue mask are determined by the header itself???(not really sure this is always the case)
        ...
        wave *CurrentWave, wave *NextWave)
{
    ...
    if (Buffer->Size > CurrentWaveRemaining) {
        OutputWave(Buffer->Beginning, CurrentWaveRemaining, CurrentWave);
        OutputWave(Buffer->Beginning + CurrentWaveRemaining, Buffer->Size - CurrentWaveRemaining, NextWave);
...
UpdateEntityResidenceAndHighRect(game_state *GameState, vec2 EntityOffset)
{
    ...
    printf("min corner: %d , %d\n", MinCorner.Chunk.X, MinCorner.Chunk.Y);
    world_position MaxCorner = AddWPosLarge(GameState->HighRectCenter, 
            Scale(+0.5f, GameState->HighRectDim));
    printf("max corner: %d , %d\n", MaxCorner.Chunk.X, MaxCorner.Chunk.Y);
...
GAME_UPDATE_AND_RENDER(GameUpdateAndRender)
{
    ...
    for (int ControllerIndex = 0; ControllerIndex < MAX_INPUT_DEVICE; ++ControllerIndex) 
    {
        ...
        if (PlayerIndex != 0) 
        {
            ...
            // speed
            PlayerHigh->dP = Sum(Scale(0.7f, PlayerHigh->dP), Scale(GameTime->Elapsed, PlayerHigh->ddP));  
            ...
            // collision detection
            r32 RemainingT = GameTime->Elapsed;
            r32 BestT = GameTime->Elapsed;
            vec2 NextSpeed = PlayerHigh->dP;
            s32 HitEntityIndex = 0;
            for (int Iteration = 0; Iteration < 4; ++Iteration) {
                BestT = RemainingT;
                for (int EntityIndex = 1; EntityIndex <= GameState->HighEntityCount; ++EntityIndex) {
                    if (EntityIndex != PlayerIndex) {
                        high_entity *TestEntityHigh = GetHighEntity(GameState, EntityIndex);
                        low_entity *TestEntityLow = GetLowEntity(GameState, TestEntityHigh->LowIndex);
                        if (TestEntityLow->Collides) {


                            // corners are chosen to include the minkowski sum
                            r32 MinX = TestEntityHigh->P.X - PlayerHigh->P.X - TestEntityLow->DimInMeters.X/2 - PlayerLow->DimInMeters.X/2;
                            r32 MaxX = TestEntityHigh->P.X - PlayerHigh->P.X + TestEntityLow->DimInMeters.X/2 + PlayerLow->DimInMeters.X/2;
                            r32 MinY = TestEntityHigh->P.Y - PlayerHigh->P.Y - TestEntityLow->DimInMeters.Y/2 - PlayerLow->DimInMeters.Y/2;
                ...
                PartialMovement = Scale(BestT , PlayerHigh->dP);

                PlayerHigh->P = Sum(PlayerHigh->P, PartialMovement);
                RemainingT -= BestT;
                PlayerHigh->dP = NextSpeed;

                if (HitEntityIndex != 0) {
                    // stairs
                    high_entity *HitEntityHigh = GetHighEntity(GameState, HitEntityIndex);
                    low_entity *HitEntityLow = GetLowEntity(GameState, HitEntityHigh->LowIndex);
                    PlayerHigh->Z += HitEntityLow->dZ;
                }
    ...
    { // update HighRectCenter
        ...
        if (GameState->CameraFollowingPlayer != 0) {
            ...
            printf("%f , %f \n", PlayerPosInHighRect.X, PlayerPosInHighRect.Y);
